...
while not interest_rate.isdigit() or float(interest_rate) == 0 \
        or not (rate_tuple[0].isdigit() and len(rate_tuple[0]) == len(rate_tuple[2]) == 1):
    interest_rate = input("이자율은 몇 %인가요? (0.0-9.9) ")
...
print("연 이자율",round(interest_rate),"%로",years,"년동안 상환")
